...
from ApiManager.models import UserInfo, ProjectInfo, ModuleInfo, TestCaseInfo, EnvInfo, TestSchema
...
@admin.register(TestSchema)
class TestSchemaAdmin(admin.ModelAdmin):
    list_display = (
        'id', 'belong_project', 's_name', 's_json', 'create_time', 'update_time')
    list_per_page = 50
    ordering = ('-create_time',)
    list_display_links = ('s_name',)
    list_filter = ('belong_project', 's_name',)  # 过滤器
    search_fields = ('belong_project', 's_name',)  # 搜索字段
    date_hierarchy = 'update_time'  # 详细时间分层筛选
