...
def edit_case(request, id=None):
    ...
    if request.session.get('login_status'):
        ...
        belong_project_id = ProjectInfo.objects.get_id_by_name(test_info[0].belong_project)
        ...
        manage_info = {
            ...
            'include': include,
            'project': ProjectInfo.objects.all().values('project_name').order_by('-create_time'),
            'edit_module_list': ModuleInfo.objects.filter(belong_project_id=belong_project_id).values('id','module_name').order_by('-create_time')
        }
