...
function copy_data_ajax(id, url) {
    ...
    if (check_case_name(data.data.name, "用例名称不能包含特殊字符.") === false) {
        return;
    }
    $.ajax({
        type: 'post',
...
function case_ajax(type, editor) {
    ...
    if (check_case_name(test.test.name.case_name, "用例名称不能包含特殊字符.") === false) {
        return;
    }
    if (type === 'edit') {
        url = '/api/edit_case/';
...
function add_row(id) {
    ...
    var cell_comparator = "<select name='test[][comparator]' class='form-control' style='height: 25px; font-size: 15px; " +
        ...
        "<option>length_less_than_or_equals</option></select>";
...
function check_case_name(newName, msg) {
    var regEn = /[`~!@#$%^&*()_+<>?:"{},.\/;'[\]]/im;
    var regCn = /[·！#￥（——）：；“”‘、，|《。》？／、【】[\]]/im;
